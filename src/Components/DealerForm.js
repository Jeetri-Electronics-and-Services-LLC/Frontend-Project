...
const EstimateForm = ({isOpen}) => {
  ...
  useEffect(() => {
    if (customerData) {
      setEstimate((prev) => ({
        ...prev,
        customername: customerData.customername || '', // New field to populate input
      }));
    }
  }, [customerData]);
  ...
  }, [orderproducts]); // Run this whenever productDetails changes
  ...
  }, [orderproducts, estimate.discount, estimate.discountMode]); // Dependencies to re-trigger calculation
  ...
  const handleProductChange = (e, index) => {
    ...
    const qty = parseFloat(updatedorderproducts[index].qty) || 0;
    const salesprice = parseFloat(updatedorderproducts[index].salesprice) || 0;
    const amount = (qty * salesprice).toFixed(2);
    updatedorderproducts[index].amount = amount;
  ...
  // Inside your DealerForm.js (EstimateForm component)
// Replace your fetchProductOrBundleDetails function with the updated one below:
...
const fetchProductOrBundleDetails = async (productName, index) => {
  try {
    ...
    let costField = '';
    // Trim and lowercase the username for comparison
    const currentUsername = localStorage.getItem('username')?.toLowerCase().trim() || 'default'; 
    switch (currentUsername) {
      case 'directdealer': costField = 'directdealerprice'; break;
      case 'promotionaldealer': costField = 'promotionaldealerprice'; break;
      case 'indirectdealer': costField = 'indirectdealerprice'; break;
      case 'promotionalindirectdealer': costField = 'promotionalindirectdelarprice'; break;
      case 'distributor': costField = 'distributorprice'; break;
      case 'promotionaldistributor': costField = 'promotionaldistributorprice'; break;
      default: costField = 'salesprice'; break;
    }

    if (productData.products) {
      const bundlePrice = productData.products.reduce(
        (total, bundleProduct) => total + parseFloat(bundleProduct.productSalesprice || 0),
        0
      );
      const bundleId = `bundle-${Date.now()}`;

      // Replace the bundle header at current index
      ...
      const newBundleItems = productData.products.map((bundleProduct) => ({
      ...
      // Insert bundle items immediately after the header
      updatedorderproducts.splice(index + 1, 0, ...newBundleItems);
    } else {
      updatedorderproducts[index] = {
        productid: productData.id,
        name: productData.name || productName,
        description: productData.description || '',
        // Use the costField determined by the username
        salesprice: productData[costField] || '0.00', 
        amount: (1 * parseFloat(productData[costField] || '0.00')).toFixed(2),
        isBundle: false,
      };
    }
  ...
  const updateCalculations = useCallback(() => {
    ...
    const taxableSubtotal = orderproducts
  ...
  }, [orderproducts, estimate.discount, estimate.discountMode, estimate.taxrates_id, estimate.salestaxes_id, estimate.customRate, estimate.deposit]);
  useEffect(() => {
    updateCalculations();
  }, [orderproducts, estimate.discount, estimate.discountMode, estimate.taxrates_id, estimate.deposit, updateCalculations]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await axios.post("http://localhost:8080/orderforms/addorderformwithlistoforderproducts", estimateData);
      alert("Dealer saved successfully!");
      console.log("Dealer saved:", response.data);
    } catch (error) {
      if (error.response) {
  ...
  const handleProductSearch = (e, index) => {
    ...
    const updatedorderproducts = [...orderproducts];
    updatedorderproducts[index].name = searchTerm;  // Update name in the productDetails state
    setorderproducts(updatedorderproducts);
  };
