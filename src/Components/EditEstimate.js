...
//     navigate('/sendemailpage', { state: { ordertype: estimate.ordertype, id: estimate.id } });
// };
...
//       if (customerData.estimateProductDetails && customerData.estimateProductDetails.length > 0) {
//         const mappedProducts = customerData.estimateProductDetails.map((product) => ({
...
//                         isBundle: false,
//         }));
//         setestimateProductDetails(mappedProducts);
//         console.log("Mapped EstimateProductDetails:", mappedProducts); // Debugging
//       }
//       if (customerData.estimateWithoutTax && customerData.estimateWithoutTax.length > 0) {
//         setEstimateWithoutTax(customerData.estimateWithoutTax);
//     }
...
//   const { name, value } = e.target;
//   setestimateProductDetails((prevDetails) => {
//       const updatedDetails = prevDetails.map((product, i) =>
//           i === index
//               ? { ...product, [name]: name === "qty" || name === "salesprice" ? parseFloat(value) || 0 : value }
//               : product
//       );
...
//     if (field === 'pricewot') {
//         const pricewotSum = estimateWithoutTax.reduce(
...
// //     // Ensure we fetch the latest salestaxes_id before submitting
// //     const updatedSalesTaxId = await fetchSalesTaxId(estimate.city_id, estimate.state_id);
// //     console.log("Updated Sales Tax ID:", updatedSalesTaxId);

// //     setEstimate((currentEstimate) => ({ ...currentEstimate, salestaxes_id: updatedSalesTaxId }));

// //     const updatedProductDetails = estimateProductDetails.map((product) => ({
// //         id: product.productid || null,  // Ensure product ID is included
// //         name: product.name || "",
// //         description: product.description || "",
// //         qty: parseFloat(product.qty) || 1,
// //         salesprice: parseFloat(product.salesprice) || 0.00,
// //         amount: parseFloat(product.amount) || 0.00,
// //         salestaxes_id: product.salestaxes_checked ? (updatedSalesTaxId || estimate.salestaxes_id || 0) : 0,
// //     }));

// //     console.log("Submitting Product Details:", updatedProductDetails);

// //     const estimateData = {
// //         ...estimate,
// //         salestaxes_id: updatedSalesTaxId || estimate.salestaxes_id || localStorage.getItem('salestaxes_id') || 0,
// //         estimateProductDetails: updatedProductDetails,
...
// //     try {
// //         const response = await axios.post(
// //             `http://localhost:8080/estimates/updateestimate/${estimate.id}`,
// //             estimateData
// //         );
...
//   if (estimateProductDetails.length === 0) {
//     alert("Cannot submit estimate without products!");
//     return;
//   }
...
//         id: product.id || null, // Ensure new products have null IDs to allow insertion
...
//       })),
//       estimateWithoutTax: estimateWithoutTax.map((item, index) => ({
//         id: item.id || index + 1,
...
//     const response = await axios.post(`http://localhost:8080/estimates/updateestimate/${estimate.id}`, estimateData);
...
//                     name: selectedProductData.name || selectedProductName,
//                     description: selectedProductData.description || '',
//                     salesprice: parseFloat(selectedProductData.salesprice) || 0.00,
//                     qty: 1,
//                     amount: parseFloat(selectedProductData.salesprice) || 0.00,
//                 }
//                 : product
//         )
//     );

//     fetchProductOrBundleDetails(selectedProductName, index);
...
//   return (
//     <form onSubmit={handleSubmit} className="estimate-form-container">
//       <h2>Edit Estimate Form</h2>
...
//           onChange={handleChange}
//            // Use readOnly if it should not be edited
//         />
...
//             <textarea name="notetocustomer" value={estimate.notetocustomer} onChange={handleChange}></textarea>
//           </div>
...
//     type="text"
//     name="salestax"
//     value={estimate.salestax} // Ensure this is bound correctly
//     readOnly
//   />
