...
const InvoiceForm = ({isOpen}) => {
      ...
      useEffect(() => {
            ...
            setInvoice((prev) => ({ ...prev, franchiseowner_id: storedUsername }));
        }, []);
      ...
      const handleSendEmail = () => {
          navigate('/emailform', { state: { invoice } });
      };
...
const [showInvoiceWithoutTax, setShowInvoiceWithoutTax] = useState(false);
      ...
      useEffect(() => {
            ...
            const fetchNoteToCustomer = async () => {
                try {
                    ...
                    setInvoice((prev) => ({ ...prev, notetocustomer: recentDescription }));
                } catch (error) {
            ...
            if (customerData) {
              setInvoice((prev) => ({
                ...
                customerdisplayname_id: customerData.customerdisplayname,
                        ...
                        ordertype: customerData.ordertype || 'Invoice',
                        // status_id: customerData.status_id || "Customer Accepted Order", 
                ...customerData, // Pre-fill invoice fields with estimate data
                ordertype: "Invoice", // Update order type to Invoice
              }));
              setProductDetails(customerData.estimateProductDetails || []);
            }
          }, [customerData]);
         ...
         const handleProductChange = (e, index) => {
            ...
            setProductDetails((prevDetails) => {
                ...
                updatedDetails[index] = {
                    ...updatedDetails[index],
                    [name]: value,
                    salestaxes_id: updatedDetails[index].salestaxes_id || localStorage.getItem('salestaxes_id') || 0
                };
            ...
            try {
                ...
                const updatedProductDetails = [...productDetails];
                  ...
                  if (productData.products) {
                        ...
                        // ✅ Add individual products inside the bundle
                        ...
                        // ✅ It's an individual product, process it
                        updatedProductDetails[index] = {
      ...
      const handleSubmit = async (e) => {
            ...
            const invoiceData = {
                ...
                taxrates_id: invoice.taxrates_id || null, // Ensure it's sent properly

                salestax: parseFloat(invoice.salestax) || 0.00,
                total: parseFloat(invoice.total) || 0.00,
                deposit: parseFloat(invoice.deposit) || 0.00,
                paycheckto: invoice.paycheckto || "",  // Ensure it's a string, not a number
                ...
                products: productDetails.map((product) => ({
                  ...
                  // salestaxes_id: product.salestaxes_id ? parseFloat(product.salestaxes_id) : parseFloat(localStorage.getItem('salestaxes_id')) || 0
              ...
              ,
                invoicewithouttax: invoiceWithoutTax.map((item, index) => ({
                  id: item.id || index + 1, // Ensure a unique ID
    ...
    setActiveSearchIndex(null);
      ...
      return (
            <form onSubmit={handleSubmit} className={`invoice-form-container ${isOpen ? 'open' : ''}`}>
                  <h2>Order Form</h2>
    ...
    <li key={idx} onClick={() => handleSelectProduct(filteredProduct.name, index)}>
        {filteredProduct.id} - {filteredProduct.name} {/* Display ID for clarity */}
            ...
            <textarea
                name="notetocustomer"
                value={invoice.notetocustomer}
                onChange={(e) => setInvoice({ ...invoice, notetocustomer: e.target.value })}
            />
                              </div>
                              <div className='invoice-form-fourth-left'>
